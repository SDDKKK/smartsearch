...
class Config:
    ...
    def _reset_model_cache_for_key(cls, key: str) -> bool:
        return key in {
            ...
            "OPENAI_COMPATIBLE_STREAM", "OPENAI_COMPATIBLE_PROVIDERS",
    ...
    def _all_config_keys(self) -> list[str]:
        ...
        for raw_key in [*os.environ.keys(), *data.keys()]:
            ...
            if self._is_openai_compatible_numbered_key(key) or self._is_openai_compatible_named_key(key):
                keys.add(key)
    ...
    def get_config_info(self) -> dict:
        ...
        explicit_main_configured = bool(
            self.xai_api_key
            or bool(self.openai_compatible_provider_configs())
        )
